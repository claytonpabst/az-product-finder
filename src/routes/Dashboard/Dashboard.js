...
class Dashboard extends Component {
    ...
    getExclusionList(){
        axios.get('/api/getExclusionList')
        .then( res => {
            log(res);

            let exclusion = '';
            res.data.forEach( obj => {
                exclusion += obj.companies + '; ';
            })
    ...
    addToExclusion = () => {
        axios.post('/api/addToExclusion', {
            newCompany: this.state.addToExclusionInput
        })
        .then( res => {
            log(res);
            let exclusion = '';
            let addToExclusionInput = '';

            res.data.forEach( obj => {
                exclusion += obj.companies + '; ';
            })

            this.setState({exclusion, addToExclusionInput})
        })
        .catch( err => {log(err)} )
    }
        ...
        return (
            <section>
                ...
                <div className="exclusion-wrapper">
                    ...
                    <input onChange={(e) => this.setState({ addToExclusionInput: e.target.value })} value={this.state.addToExclusionInput} />
